...
path = HERE / "modules" / "try-guix" / "omarchy-menu.py"
...
class OmarchyMenuTests(unittest.TestCase):
    ...
    def test_no_arch_is_left(self):
        for word in ("pacman", "yay", "aur", "ghostty", "update.omarchy"):
            self.assertNotIn(word, self.text.lower().replace("try-guix", ""))

    def test_guix_packages_and_system(self):
        self.assertIn("try-guix-pkg pick-add", self.menu["install.package"]["action"])
        self.assertIn("try-guix-pkg pick-remove", self.menu["remove.package"]["action"])
        self.assertEqual(self.menu["install.terminal.alacritty"]["checked"],
                         "try-guix-pkg present alacritty")
        self.assertIn("sudo try-guix-pkg add emacs-pgtk",
                      self.menu["install.editor.emacs"]["action"])
        self.assertIn("try-guix-reconfigure", self.menu["update.system.apply"]["action"])
...
class OmarchyPackageTests(unittest.TestCase):
    def test_menu_guard_asks_guix(self):
        ...
        source = (HERE / "modules/try-guix/omarchy.scm").read_text()
