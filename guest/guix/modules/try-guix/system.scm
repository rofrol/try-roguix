;;; Try Guix — the guest's operating system.
;;;
;;; `try-guix-operating-system' is the whole system; guest/guix/system.scm
...
;;; by Omarchy's install menus through try-guix-pkg), and
;;; `sudo try-guix-reconfigure' applies it with the Guix that built the image.
(define-module (try-guix system)
  #:use-module (gnu)
  ...
  #:use-module (try-guix integrations)
  #:use-module (try-guix omarchy)
  #:use-module (try-guix packages)
  #:use-module (try-guix services)
  #:export (try-guix-operating-system
            %pinned-guix-root))
...
(define %pinned-guix-root "/var/guix/gcroots/try-guix-guix")
(define (pinned-guix)
...
;; Try Guix: this machine's system configuration.
;;
;; Add programs by their Guix package names (`guix search NAME' finds them),
;; then apply the change with:
;;
;;   sudo try-guix-reconfigure
;;
;; Omarchy's install menus edit the same list. Everything else is the Try Guix
;; system in /etc/try-guix, which this reuses as is.
...
(define try-guix-config-activation
...
;; items, so a reconfigure, which grafts again, would rebuild Try Guix's own
...
;; system as a GC root.
(define %ungrafted-root "/var/guix/gcroots/try-guix-ungrafted")
...
(define* (try-guix-operating-system #:key (packages '()))
  "Return the Try Guix system, adding PACKAGES, a list of package names."
  ...
  (operating-system
    ...
    (services
     (cons* ;; The same definitions for in-guest reconfigure and rollback, so they
            ...
            (simple-service 'try-guix-config activation-service-type
                            try-guix-config-activation)
