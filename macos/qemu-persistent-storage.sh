...
# The guest is a GPT disk booted by the runtime's UEFI firmware. Its state
# lives in a `guix` subdirectory of the state root, where Try Roguix has always
# kept it (the root itself once held Try Omarchy's Arch VM).
...
# Support/Try Roguix" and for anything under /Volumes.
...
_qps_prepare_state_root() {
  ...
  if [[ -n ${OMARCHY_QEMU_GPU_STATE_ROOT:-} ]]; then
    ...
    qps_configured_root="$HOME/Library/Application Support/Try Roguix/VM/v1"
  fi
...
_qps_require_compatible_workspace() {
  ...
  if [[ $QPS_METADATA_SCHEMA != "$QEMU_PERSISTENT_STORAGE_SCHEMA" ]]; then
    _qps_incompatible \
      'the saved VM uses an unsupported storage format; use Reset Roguix to continue'
    return $?
  fi
...
_qps_migrate_legacy_single_workspace() {
  ...
  if [[ -e $qps_final || -L $qps_final ]]; then
    ...
    if [[ $qps_mode == persistent ]] && \
      ...
      _qps_incompatible \
        'multiple saved VMs were found; use Reset Roguix to return to one supported disk'
      return $?
    fi
    return 0
  fi
...
_qps_select_persistent_disk() {
  ...
  if _qps_publish_recorded_selection "$qps_final" "$qps_source"; then
    return 0
  else
    qps_status=$?
    qemu_persistent_storage_release_lock
...
qemu_persistent_storage_select_existing() {
  ...
  if _qps_publish_recorded_selection "$qps_final"; then
    return 0
  else
    qps_status=$?
    qemu_persistent_storage_release_lock
