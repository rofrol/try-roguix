...
final class CoreAudioDeviceChangeMonitor {
    private let queue = DispatchQueue(label: "dev.tryroguix.native.audio-devices")
...
final class NativeAudioBridge {
    ...
    private let stateQueue = DispatchQueue(label: "dev.tryroguix.native.audio-bridge-state")
    private let writeQueue = DispatchQueue(label: "dev.tryroguix.native.audio-bridge-writes")
