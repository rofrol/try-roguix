...
final class NetworkEditor: NSObject {
    ...
    init(preferences: VMNetworkPreferences, interfaces: [VMBridgeInterface],
        ...
        alert.informativeText = "Changes apply the next time you start Roguix."
    ...
    @objc private func update() {
        ...
        explanation.stringValue = compatibilityRequired
            ? "Wi-Fi bridging on this Mac temporarily adjusts DHCP handling for all bridged VMs, including other virtualization apps. The previous setting is restored when Roguix stops. Saving this choice enables that handling automatically."
        ...
        detail.stringValue = bridged
            ...
            : "Roguix uses your Mac’s network connection. No additional setup is needed. Saved port-forwarding rules apply in this mode."
        if bridged && !valid { detail.stringValue = "The selected adapter is unavailable. Roguix will start offline and connect automatically when this adapter returns." }
    ...
    @objc private func removeService() {
        ...
        confirmation.informativeText = "This stops networking for any active bridged Roguix VM. Shut those VMs down first. You can set up the helper again later."
